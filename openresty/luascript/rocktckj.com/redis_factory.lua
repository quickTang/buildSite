...
local redis_factory = function(h)
    ...
    h.cosocket_pool   = {max_idel = 50000, size = 4000}
    ...
    self.destruct = function()
        ...
        for name, _ in pairs(self.pool) do
            local redisInstance=ngx.ctx[name].redis_instance;
        if(redisInstance ~= nil and redisInstance ~= ngx.null and redisInstance ~= '') then
            local ok, msg = redisInstance:set_keepalive(
